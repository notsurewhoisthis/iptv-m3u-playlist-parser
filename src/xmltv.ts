...
export function parseXmltv(xml: string): { channels: TvgChannel[] } {
  ...
  for (const ch of channelsArr) {
    ...
    const namesArr = Array.isArray(namesRaw)
      ? namesRaw
      : namesRaw
        ? [namesRaw]
        : [];
    const displayNames: string[] = namesArr
      .map((n: any) =>
        typeof n === "string" ? n : (n?.["#text"] ?? "").toString(),
      )
      .map((s: string) => s.trim())
      .filter(Boolean);
...
export function buildChannelCategoryMap(
